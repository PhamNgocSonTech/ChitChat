...
describe('GET /api/room', () => {
    it('should return an array of rooms', async () => {
        const response = await request.get('/api/room/getAllRoom').set('Authorization', token);
    ...
    it('should get the room by room id', async () => {
        const response = await request
            .get(`/api/room/getRoom/${roomId.toString()}`)
            .set('Authorization', token);
...
describe('PUT /api/room/update/:room_name', () => {
...
describe('DELETE /:room_name', () => {
    it('should delete a room based on the name', async () => {
        const room_name = 'Jest Test Room';
        let response = await request.delete(`/api/room/deleteRoom/${room_name}`).set('Authorization', token);

        expect(response.status).toEqual(200);
        expect(Object.keys(response.body).length).toBeGreaterThan(0);

        response = await request.get('/api/room/getAllRoom').set('Authorization', token);
