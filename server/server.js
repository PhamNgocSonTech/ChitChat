    ...
    socket.on('disconnect', async() => {
        if(currentRoomId) {
            ...
            socket.on('userTyping', data => {
                if(!userTyping[data.room._id]){
                    userTyping[data.room._id] = [];
                }else{
                    if(!userTyping[data.room._id].includes(data.user.handle)){
                        userTyping[data.room._id].push(data.user.handle);
                    }
                }
                socket.broadcast
                    .to(data.room._id)
                    .emit('receivedUserTyping', JSON.stringify(userTyping[data.room._id]))
            })
